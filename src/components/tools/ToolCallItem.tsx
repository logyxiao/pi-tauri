...
export function ToolCallItem({ tool, onSelect }: ToolCallItemProps) {
  ...
  return (
    <div>
      <button
        className="group flex w-full items-center gap-2 rounded-none border border-border bg-background/60 px-2 py-1.5 text-left text-[11px] transition hover:bg-muted/70"
        onClick={() => {
      ...
      >
        ...
        <span className="w-12 shrink-0 truncate font-mono font-semibold text-foreground">{tool.name}</span>
        ...
        <span
          className={cn(
            "rounded-none px-1.5 py-0.5 text-[9px] font-medium",
