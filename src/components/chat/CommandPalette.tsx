...
export function CommandPalette({ commands, query, selectedIndex, onSelect }: CommandPaletteProps) {
  ...
  return (
    <div className="overflow-hidden rounded-none border border-border bg-surface/95 shadow-xl backdrop-blur-[2px]">
      ...
      <div className="max-h-[min(18rem,42vh)] overflow-auto overscroll-contain p-1.5">
        {filtered.map((command, index) => (
          <button
            key={command.name}
            className={cn(
              "flex w-full min-w-0 items-start gap-3 rounded-none px-3 py-2 text-left font-mono text-xs transition hover:bg-muted",
